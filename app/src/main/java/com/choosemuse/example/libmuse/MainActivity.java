...
public class MainActivity extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        streamReader.manager = MuseManagerAndroid.getInstance();
        streamReader.manager.setContext(this);
        ...
        streamReader.connectionListener = new ConnectionListener(weakActivity);
        // Register a listener to receive data from a Muse.
        streamReader.dataListener = new DataListener(weakActivity);
        // Register a listener to receive notifications of what Muse headbands
        // we can connect to.
        streamReader.manager.setMuseListener(new MuseL(weakActivity));
        ...
        streamReader.handler.post(tickUi);
    }
    ...
    protected void onPause() {
        ...
        streamReader.manager.stopListening();
    }
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.refresh) {
            ...
            streamReader.manager.stopListening();
            streamReader.manager.startListening();
            ...
            List<Muse> availableMuses = streamReader.manager.getMuses();
            ...
            if (availableMuses.size() < 1 || musesSpinner.getAdapter().getCount() < 1) {
                ...
                streamReader.muse = availableMuses.get(musesSpinner.getSelectedItemPosition());
                // Unregister all prior listeners and register our data listener to
                // receive the MuseDataPacketTypes we are interested in.  If you do
                // not register a listener for a particular data type, you will not
                // receive data packets of that type.
                streamReader.muse.unregisterAllListeners();
                streamReader.muse.registerConnectionListener(streamReader.connectionListener);
                streamReader.muse.registerDataListener(streamReader.dataListener, MuseDataPacketType.EEG);
                streamReader.muse.registerDataListener(streamReader.dataListener, MuseDataPacketType.ALPHA_RELATIVE);
                streamReader.muse.registerDataListener(streamReader.dataListener, MuseDataPacketType.BETA_RELATIVE);
                streamReader.muse.registerDataListener(streamReader.dataListener, MuseDataPacketType.ACCELEROMETER);
                streamReader.muse.registerDataListener(streamReader.dataListener, MuseDataPacketType.BATTERY);
                streamReader.muse.registerDataListener(streamReader.dataListener, MuseDataPacketType.DRL_REF);
                streamReader.muse.registerDataListener(streamReader.dataListener, MuseDataPacketType.QUANTIZATION);

                // Initiate a connection to the headband and stream the data asynchronously.
                streamReader.muse.runAsynchronously();
            }
            ...
            if (streamReader.muse != null) {
                streamReader.muse.disconnect(false);
            }
            ...
            if (streamReader.muse != null) {
                streamReader.dataTransmission = !streamReader.dataTransmission;
                streamReader.muse.enableDataTransmission(streamReader.dataTransmission);
            }
        }
    }
    ...
    public void museListChanged() {
        final List<Muse> list = streamReader.manager.getMuses();
    ...
    public void receiveMuseConnectionPacket(final MuseConnectionPacket p, final Muse muse) {
        ...
        streamReader.handler.post(new Runnable() {
            @Override
            public void run() {
    ...
    public void receiveMuseDataPacket(final MuseDataPacket p, final Muse muse) {
        ...
        switch (p.packetType()) {
            case EEG:
                getEegChannelValues(streamReader.eegBuffer,p);
                streamReader.eegStale = true;
                break;
            case ACCELEROMETER:
                getAccelValues(p);
                streamReader.accelStale = true;
                break;
            case ALPHA_RELATIVE:
                getEegChannelValues(streamReader.alphaBuffer,p);
                streamReader.alphaStale = true;
                break;
            case BETA_RELATIVE:
                getEegChannelValues(streamReader.betaBuffer,p);
                streamReader.betaStale = true;
            case BATTERY:
            case DRL_REF:
            case QUANTIZATION:
            default:
    ...
    private void getAccelValues(MuseDataPacket p) {
        streamReader.accelBuffer[0] = p.getAccelerometerValue(Accelerometer.X);
        streamReader.accelBuffer[1] = p.getAccelerometerValue(Accelerometer.Y);
        streamReader.accelBuffer[2] = p.getAccelerometerValue(Accelerometer.Z);
    }
    ...
    private void initUI() {
        ...
        Button graphButton = (Button) findViewById(R.id.b_graph);
        graphButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(getWindow().getContext(), GraphActivity.class));
            }
        });
    ...
    private final Runnable tickUi = new Runnable() {
        @Override
        public void run() {
            if (streamReader.eegStale) {
                updateEeg();
            }
            if (streamReader.accelStale) {
                updateAccel();
            }
            if (streamReader.alphaStale) {
                updateAlpha();
            }
            if (streamReader.betaStale) {
                updateBeta();
            }
            streamReader.handler.postDelayed(tickUi, 1000 / 60);
        }
    };
    ...
    private void updateAccel() {
        ...
        acc_x.setText(String.format("%6.2f", streamReader.accelBuffer[0]));
        acc_y.setText(String.format("%6.2f", streamReader.accelBuffer[1]));
        acc_z.setText(String.format("%6.2f", streamReader.accelBuffer[2]));
    }
    ...
    private void updateEeg() {
        ...
        tp9.setText(String.format("%6.2f", streamReader.eegBuffer[0]));
        fp1.setText(String.format("%6.2f", streamReader.eegBuffer[1]));
        fp2.setText(String.format("%6.2f", streamReader.eegBuffer[2]));
        tp10.setText(String.format("%6.2f", streamReader.eegBuffer[3]));
    }

    private void updateAlpha() {
        TextView elem1 = (TextView)findViewById(R.id.elem1);
        elem1.setText(String.format("%6.2f", streamReader.alphaBuffer[0]));
        TextView elem2 = (TextView)findViewById(R.id.elem2);
        elem2.setText(String.format("%6.2f", streamReader.alphaBuffer[1]));
        TextView elem3 = (TextView)findViewById(R.id.elem3);
        elem3.setText(String.format("%6.2f", streamReader.alphaBuffer[2]));
        TextView elem4 = (TextView)findViewById(R.id.elem4);
        elem4.setText(String.format("%6.2f", streamReader.alphaBuffer[3]));
    }

    void updateBeta(){
        TextView elem1 = (TextView)findViewById(R.id.beta_elem1);
        elem1.setText(String.format("%6.2f", streamReader.betaBuffer[0]));
        TextView elem2 = (TextView)findViewById(R.id.beta_elem2);
        elem2.setText(String.format("%6.2f", streamReader.betaBuffer[1]));
        TextView elem3 = (TextView)findViewById(R.id.beta_elem3);
        elem3.setText(String.format("%6.2f", streamReader.betaBuffer[2]));
        TextView elem4 = (TextView)findViewById(R.id.beta_elem4);
        elem4.setText(String.format("%6.2f", streamReader.betaBuffer[3]));
    }
